...
4. WHEN the new Brewfile hash matches the hash stored in state.json THEN the system SHALL skip formatting and backup, displaying "Brewfile unchanged, skipping backup"
...
3. WHEN requesting formatting THEN the system SHALL instruct Anthropic to add a header noting the file was generated by `den`
4. WHEN requesting formatting THEN the system SHALL instruct Anthropic to add a one-liner description comment for each package
5. WHEN requesting formatting THEN the system SHALL instruct Anthropic to organize packages into logical categories with section headers
